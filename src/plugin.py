...
def restore_context(params=None, *_):
    ...
    # Load windows metadata (stub for now)
    windows_data = read_json(context_path / "windows.json")
    logging.info(f"Window restore stub: {len(windows_data)} windows loaded.")
...
def write_response(response: Response) -> None:
    try:
        ...
        windll.kernel32.WriteFile(pipe, message_bytes, message_len, wintypes.DWORD(), None)
    except Exception as e:
